...
public class Act16Arrays2 {
	...
	public static void main(String[] args) {
		...
		for (int fila = 0; fila < calificaciones.length; fila++) {
			...
			for (int columna = 0; columna < calificaciones[0].length; columna++) {
		...
		System.out.println("\n");
		System.out.println("La nota media del curso " + total / calificaciones.length);
		for (int fila = 0; fila < calificaciones.length; fila++) {
			if (Float.parseFloat(calificaciones[fila][1]) > (total / calificaciones.length)) {
System.out.println("Estos osn los que han sacado nota por encima de la media "+calificaciones[fila][0]+ " "+calificaciones[fila][1]);
			}
		}
	}
